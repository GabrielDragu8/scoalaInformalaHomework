...
const updateDisplay = () => {
  ...
  characterNameDisplay.textContent = characterName || "Character";
};

const updateCharacterName = () => {
  const name = document.querySelector('#name');
  // const characterNameDisplay = document.querySelector('#characterNameDisplay');
  name.addEventListener('input', updateDisplay);
...
const updateInventory = () => {
  ...
  inventory.forEach(item => {
    ...
    let itemTotalWeight = Math.round(item.weight * item.qty * 100) / 100;
    let itemTotalValue = Math.round(item.cost * item.qty * 100) / 100;
...
const updateTotalQuantity = (totalWeight) => {
  ...
  document.querySelector('.total-weight').textContent = parseFloat(totalWeight);
  document.querySelector('.total-weight-inventory').textContent = parseFloat(totalWeight);
}

const updateTotalValue = (totalValue) => {
  document.querySelector('.total-value').textContent = parseFloat(totalValue);
  document.querySelector('.total-value-inventory').textContent = parseFloat(totalValue);
}
...
const saveCharacterData = () => {
  const characterData = {
    ...
    strScore: parseInt(document.querySelector("#strScore").value),
    size: document.querySelector("#size").value,
    multiplier: parseInt(document.querySelector('#multiplier').value),
    inventory: inventory
  };
...
const loadCharacterData = () => {
  ...
  if (characterData) {
    ...
    document.querySelector("#strScore").value = characterData.strScore;
    document.querySelector("#size").value = characterData.size;
    document.querySelector('#multiplier').value = characterData.multiplier;
...
document.querySelectorAll('#name, #strScore, #size, #multiplier').forEach(element => {
...
function sortInventory() {
  const inventoryContainer = document.getElementById('inventory');
  const items = Array.from(inventoryContainer.children);
  items.sort((a, b) => a.textContent.localeCompare(b.textContent));
  items.forEach(item => inventoryContainer.removeChild(item));
  items.forEach(item => inventoryContainer.appendChild(item));
}

document.getElementById('sortInventoryButton').addEventListener('click', sortInventory);
